...
const OrderScreen = ({ match, history }) => {
  ...
  useEffect(() => {
    ...
    const addPayPalScript = async () => {
      ...
      script.src = `https://www.paypal.com/sdk/js?client-id=${clientId}` //from the paypal website
      script.async = true
      // once this loads, setSdkReady to true. we want this to tell us if the paypal script has been loaded
      script.onload = () => {
        setSdkReady(true)
      }
      // then we need to add the script to the body
    ...
    if (!order || successPay || successDeliver) {
      ...
      if (!window.paypal) {
        // order is not paid, addPayPalscript to load the paypal script
        addPayPalScript()
      } else {
        // if order is not paid and paypal window is displayed, then setSdkReady to true - meaning the paypal window has been loaded
        setSdkReady(true)
      }
    }
